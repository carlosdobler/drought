    ...
    map(dd, \(d) {
      ss_var <-
        ...
        map(\(v) {
          ...
          system(str_glue("gcloud storage cp {f} {dir_tmp}"), ignore.stdout = T, ignore.stderr = T)
...
walk(df_sources$model |> set_names(), \(mod) {
  ...
  s_1model <-
    imap(vars_nmme, \(var, v) {
      ...
      var_l <-
        case_when(var == "prec" ~ "precipitation", var == "tref" ~ "average-temperature")
          ...
          s_bias_adj_1mon <-
            ...
            map(seq(dim(fcst_1mon)[3]), \(mem) {
              ...
              nmme_quantile <-
                ...
                st_warp(st_as_stars(
                  st_bbox(
                    c(xmin = -180.125, ymin = -90.125, xmax = 179.875, ymax = 90.125),
                    crs = 4326
                  ),
                  dx = 0.25
                )) |>
