...
land_mask <- function(stars_obj){ # reference grid
...
load_data <- function(dir_origin_cloud, # bucket dir where all vars are located
                      dir_dest_local,
                      date_vector){
...
distr_params <- function(s,
  ...
  walk(seq(12), \(mon){
    ...
    f <- 
      str_glue("{dir_tmp_local}/{f_name_root}_{str_pad(mon, 2, 'left', '0')}.nc")
