...
dir_tmp <- "/mnt/pers_disk/drought_w_nex"
...
s_z <-
  rt_gs_download_files(
    "gs://clim_data_reg_useast1/era5/era5_geopotential.nc",
    dir_tmp,
    quiet = T
  ) |>
  read_ncdf() |>
...
s_gamma <-
  s_z |>
  mutate(P = 101.3 * ((293 - 0.0065 * z) / 293)^5.26, gamma = 1.013e-3 * P / (0.622 * 2.45)) |>
  select(gamma)
...
vars <- c("tasmax", "tasmin", "hurs", "sfcWind", "rsds", "pr")

yrs <- seq(1970, 2099)

df_nex_files <-
  read_csv("drought_w_nex/df_nex_files.csv") |>
  filter(year >= first(yrs), year <= last(yrs)) |>
...
for (model in models |> tail(-19)) {
  ...
  download.file(s_ref, str_glue("{dir_tmp}/{fs::path_file(s_ref)}"), method = "wget", quiet = T)
  ...
  walk(yrs, \(yr) {
    ...
    f_vars <-
      future_map(vars |> set_names(), \(var) {
        ...
        download.file(f, str_glue("{dir_tmp}/{fs::path_file(f)}"), method = "wget", quiet = T)
    ...
    s_vars <-
      ...
      future_imap(\(f, i) {
        ...
        if (class(un) == "try-error" & (i == "tasmin" | i == "tasmax")) {
          message("no units...")
          un <- "K"
          s <-
            s |>
            mutate(!!sym(i) := units::set_units(!!sym(i), !!un))
        }
    ...
    s_PET <-
      ...
      map(\(mon) {
        ...
        # 1 month
        s_vars_f <-
          s_vars[which(names(s_vars) != "pr")] |> # all vars not pr
          map(slice, time, mon) |>
          c(
            z = list(s_z_nex_grid),
        ...
        s_vars_f <-
          ...
          mutate(
            ...
            Rnl = 4.903e-9 *
              ((tasmax + 273.16)^4 + (tasmin + 273.16)^4) /
              2 *
              (0.34 - 0.14 * sqrt(ea)) *
              (1.35 * rsds / Rso - 0.35),
        ...
        s_ET <-
          ...
          mutate(
            numerator = 0.408 *
              delta *
              (Rn - G) +
              gamma * (900 / (tasmean + 273)) * sfcWind * (es - ea),
    ...
    str_glue(
      "gcloud storage mv {f_res} gs://clim_data_reg_useast1/nex/monthly_aggregates/potential_evapotranspiration_pm/{model}/"
    ) |>
      system(ignore.stdout = T, ignore.stderr = T)
    ...
    str_glue(
      "gcloud storage mv {f_res} gs://clim_data_reg_useast1/nex/monthly_aggregates/water_balance_pm/{model}/"
    ) |>
      system(ignore.stdout = T, ignore.stderr = T)
    ...
    f_res <- str_glue("{dir_tmp}/ai-pm_yr_{model}_NEX_{yr}.nc")
    ...
    str_glue(
      "gcloud storage mv {f_res} gs://clim_data_reg_useast1/nex/annual_aggregates/aridity_index_pm/{model}/"
    ) |>
      system(ignore.stdout = T, ignore.stderr = T)
