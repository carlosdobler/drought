...
model_dirs <- 
  rt_gs_list_files("gs://clim_data_reg_useast1/nex/monthly_aggregates/water-balance-pm/")
...
walk(model_dirs, \(model_dir){
  ...
  pwalk(df_tiles_land, function(tile_id, start_x, start_y, count_x, count_y, ...){
    ...
    s_tile <-
      ff |> 
      future_map(\(f) rt_tile_load(start_x, start_y, count_x, count_y, f, parallel = F))
    ...
    walk(int_windws, \(windw){
      ...
      st_dimensions(s_wb_q)[3] <- st_dimensions(s_tile)[3]
      
      s_wb_q |> 
        rt_write_nc(str_glue("{dir_tiles}/tile_w{windw}_{tile_id}.nc"))
  ...
  ff_tiles <- 
    map(int_windws |> set_names(), \(windw){
      dir_tiles |>
        fs::dir_ls(regexp = str_glue("_w{windw}_"))
    })
  ...
  yrs_vector |>
    ...
    walk(\(yr) {
      ...
      iwalk(ff_tiles, \(f, windw){
        ...
        mos <- 
          rt_tile_mosaic_gdal(f,
                              dir_mos,
                              spatial_dims = st_dimensions(s_proxy),
                              time_dim = seq(as_date(str_glue("{yr}-01-01")),
                                             as_date(str_glue("{yr}-12-01")),
                                             by = "1 month"),
                              time_full = full_time_vector) |> 
          setNames("wb-quantile")
        
        f_res <- str_glue("{dir_mos}/wb-pm-quantile_w{windw}_bl-{str_flatten(range(baseline), '-')}_mon_{model}_NEX_{yr}.nc")
        
        rt_write_nc(mos,
                    f_res,
                    gatt_name = "source code",
                    gatt_val = "https://github.com/carlosdobler/drought/tree/main/drought_w_nex")
        
        system(str_glue("gcloud storage mv {f_res} gs://clim_data_reg_useast1/nex/monthly_aggregates/water_balance_pm_quantile/{model}/"),
